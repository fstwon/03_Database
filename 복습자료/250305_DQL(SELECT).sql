...
-- 이름, 생년월일, 나이 정보 조회
SELECT NAME 이름, BIRTHDATE 생년월일, EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM BIRTHDATE) 나이 FROM CUSTOMER;

-- 나이가 40대인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM BIRTHDATE) BETWEEN 40 AND 49;

-- 광역시에 거주 중인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE INSTR(ADDRESS, '광역시') > 0;
...
-- 본인이 태어난지 며칠 째인지 확인
SELECT ROUND(SYSDATE - TO_DATE('250301', 'YYMMDD')) || '일 째' "인생" FROM DUAL;
...
SELECT EXTRACT(MONTH FROM HIRE_DATE) || '월' "입사 월", COUNT(*) FROM EMPLOYEE  GROUP BY EXTRACT(MONTH FROM HIRE_DATE) ORDER BY EXTRACT(MONTH FROM HIRE_DATE);
