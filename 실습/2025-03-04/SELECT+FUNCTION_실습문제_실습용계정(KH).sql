...
SELECT EMP_NAME, SALARY * 12, NVL((SALARY + SALARY * BONUS) * 12, 0), NVL((SALARY + SALARY * BONUS) * 12 - ((SALARY * 12) * 0.03), 0) FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, NVL((SALARY + SALARY * BONUS) * 12 - ((SALARY * 12) * 0.03), 0), HIRE_DATE 
FROM EMPLOYEE
...
WHERE 
    ...
    (DEPT_CODE IN ('D9', 'D6') AND HIRE_DATE BETWEEN '90/01/01' AND '00/12/01')
    AND SALARY >= 2700000;

-- 15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT EMP_NAME, EMP_NO, SUBSTR(EMP_NO, 1, 2), SUBSTR(EMP_NO, 3, 2), SUBSTR(EMP_NO, 5, 2) FROM EMPLOYEE;
...
-- 입사일부터 오늘까지 일 수, SYSDATE - HIRE_DATE
-- 개월 * 30
-- HIRE_DATE, SYSDATE
SELECT EMP_NAME, ROUND(ABS(HIRE_DATE - SYSDATE)) "근무일수2", ROUND(SYSDATE - HIRE_DATE) "근무일수2"  FROM EMPLOYEE;

-- 18. EMPLOYEE테이블에서 사번이 홀수인 직원들의 정보 모두 조회
SELECT * FROM EMPLOYEE WHERE MOD(EMP_ID, 2) = 1;

-- 19. EMPLOYEE테이블에서 근무 년수가 20년 이상인 직원 정보 조회
-- SELECT * FROM EMPLOYEE WHERE (SYSDATE - HIRE_DATE + 1) / 365 >= 20;
...
--    DEPT_CODE,
    -- 'YYYY"년" MM"월" DD"일"'
    -- TO_CHAR(SUBSTR(EMP_NO, 1, 6))
    -- TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6)), 'YYYY"년" MM"월" DD"일"')
    -- TO_CHAR(TO_NUMBER(SUBSTR(EMP_NO, 1, 6)), 'YYYY')
    -- TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6)), 'YYYY') 생년월일
    -- TO_CHAR(EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM SUBSTR(EMP_NO, 1, 6))) 나이
    -- EXTRACT(MONTH FROM (SUBSTR(EMP_NO, 1, 6)))
 --   SYSDATE,
--    TO_CHAR('62/67/95', 'YY"년" MM"월" DD"일"')
--FROM EMPLOYEE;
-- WHERE EMP_ID != 215;
...
--     주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME 사원명, SUBSTR(EMP_NO, 1, 6) 주민번호, 
...
--     (단, 최근 날짜 순서로 정렬)
SELECT COUNT(*) 전체직원수, COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2001, 1)) "2001년",
...
FROM EMPLOYEE 
WHERE EXTRACT(YEAR FROM HIRE_DATE) BETWEEN 2001 AND 2004;
